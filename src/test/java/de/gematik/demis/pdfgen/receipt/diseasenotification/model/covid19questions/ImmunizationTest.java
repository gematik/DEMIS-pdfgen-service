...
class ImmunizationTest {
  ...
  void builderAndGetters_shouldHandleNullAndBlankGracefully() {
    ...
    List<String> vaccines = Arrays.asList(null, "", "  ", "kind1");
              ...
              occurrenceDate -> {
                notes.forEach(
                    note -> {
                      ...
                      // then
                      assertThat(immunization.getVaccineCodeText()).isEqualTo(vaccineCodeText);
                      assertThat(immunization.getOccurrenceDate()).isEqualTo(occurrenceDate);
                      assertThat(immunization.getNote()).isEqualTo(note);
                    });
              });
        });
  }
