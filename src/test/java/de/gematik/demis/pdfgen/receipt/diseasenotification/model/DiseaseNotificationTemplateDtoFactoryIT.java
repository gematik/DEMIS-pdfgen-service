...
@SpringBootTest
class DiseaseNotificationTemplateDtoFactoryIT {

  @Autowired private DiseaseNotificationTemplateDtoFactory dtoFactory;

  @Test
  void create_shouldHandleNullGracefully() {
    assertThat(dtoFactory.create(null)).isNull();
  }

  @Test
  void create_shouldTestFactoryCreation() {
    // given
    DiseaseNotificationTemplateDto dto = dtoFactory.create(createDiseaseNotificationBundle());
    ...
    assertThat(dto.getNotifier().getFacility().getName()).isEqualTo("TEST Organisation");
    ...
    assertThat(dto.getCommonQuestions()).isNotNull();
    assertThat(dto.getCovid19Questions()).isNotNull();
  }
}
