...
class LabTestFactoryIT {
  ...
  void createLabTests_createLabTestsAsExpected() {
    ...
    assertThat(labTest.getInterpretation()).isEqualTo(LabTestInterpretationEnum.POSITIVE);
    assertThat(labTest.getMethod()).isEqualTo("Nucleic acid assay (procedure)");
    assertThat(labTest.getNotes()).hasSize(1);
    assertThat(labTest.getNotes().get(0)).isEqualTo("Nette Zusatzinformation …");
