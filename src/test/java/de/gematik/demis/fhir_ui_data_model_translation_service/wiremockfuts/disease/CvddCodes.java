package de.gematik.demis.fhir_ui_data_model_translation_service.wiremockfuts.disease;

/*-
 * #%L
 * pdfgen-service
 * %%
 * Copyright (C) 2025 gematik GmbH
 * %%
 * Licensed under the EUPL, Version 1.2 or - as soon they will be approved by the
 * European Commission – subsequent versions of the EUPL (the "Licence").
 * You may not use this work except in compliance with the Licence.
 *
 * You find a copy of the Licence in the "Licence" file or at
 * https://joinup.ec.europa.eu/collection/eupl/eupl-text-eupl-12
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the Licence is distributed on an "AS IS" basis,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either expressed or implied.
 * In case of changes by gematik find details in the "Readme" file.
 *
 * See the Licence for the specific language governing permissions and limitations under the Licence.
 *
 * *******
 *
 * For additional notes and disclaimer from gematik and in case of changes by gematik find details in the "Readme" file.
 * #L%
 */

import de.gematik.demis.fhir_ui_data_model_translation_service.wiremockfuts.Feature;
import de.gematik.demis.fhir_ui_data_model_translation_service.wiremockfuts.FeatureCollection;
import de.gematik.demis.fhir_ui_data_model_translation_service.wiremockfuts.codes.ValueSetFeature;
import java.util.Collection;

final class CvddCodes extends FeatureCollection {

  private static Feature createCvddVaccines() {
    ValueSetFeature cvddVaccines =
        new ValueSetFeature("https://demis.rki.de/fhir/ValueSet/vaccineCVDD");
    cvddVaccines.add(
        "EU/1/20/1528",
        """
                                                                {
                                                                    "code": "EU/1/20/1528",
                                                                    "display": "Comirnaty"
                                                                }""");
    cvddVaccines.add(
        "otherVaccine",
        """
                                                                {
                                                                    "code": "otherVaccine",
                                                                    "display": "Anderer Impfstoff"
                                                                }""");
    return cvddVaccines;
  }

  @Override
  protected void add(Collection<Feature> features) {
    features.add(createCvddVaccines());
  }
}
