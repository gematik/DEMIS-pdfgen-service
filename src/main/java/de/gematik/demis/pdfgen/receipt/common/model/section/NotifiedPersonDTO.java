...
@ToString
@EqualsAndHashCode
public class NotifiedPersonDTO implements TelecomsHolder {
  ...
  public List<AddressTemplateAdapter> getAddresses() {
    final Stream<AddressTemplateAdapter> fromAddresses =
        Objects.requireNonNullElse(addressDTOs, List.<AddressDTO>of()).stream()
            .map(AddressTemplateAdapter::from);
    final Stream<AddressTemplateAdapter> fromOrganizations =
        Objects.requireNonNullElse(organizationDTOs, List.<OrganizationDTO>of()).stream()
            .map(AddressTemplateAdapter::from);

    return Stream.concat(fromAddresses, fromOrganizations)
        .sorted(AddressTemplateAdapter.naturalOrder())
        .toList();
  }
}
